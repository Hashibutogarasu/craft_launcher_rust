pub mod modern_fabric {
    ...
    // Represents a Modern Fabric manifest file
    // This structure corresponds to the JSON manifest for Modern Fabric loader
    #[derive(Debug, Clone, Serialize, Deserialize)]
    pub struct ModernFabricManifest {
        ...
        /// Arguments to pass to JVM and game
    ...
    // Represents arguments for JVM and game
    #[derive(Debug, Clone, Serialize, Deserialize)]
    ...
    // Functions for working with Modern Fabric manifests
    impl ModernFabricManifest {
        /// Parse a Modern Fabric manifest from JSON string
        ...
        /// Get the Fabric loader version from libraries
        pub fn get_loader_version(&self) -> Option<&str> {
            self.libraries
                .iter()
                .find(|lib| lib.name.starts_with("net.fabricmc:fabric-loader:"))
                .map(|lib| {
                    let parts: Vec<&str> = lib.name.split(':').collect();
                    parts.get(2).map(|v| *v).unwrap_or("")
                })
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_parse_modern_fabric_manifest() {
        // Sample JSON based on the attached fabric-loader-0.16.14-1.21.2.json
        let sample_json = r#"{
            "inheritsFrom": "1.21.2",
            "releaseTime": "2025-05-20T11:27:17+0000",
            ...
            "libraries": [
                ...
                {
                    "name": "net.fabricmc:intermediary:1.21.2",
                    "url": "https://maven.fabricmc.net/"
                },
                {
            ...
            "id": "fabric-loader-0.16.14-1.21.2",
            "time": "2025-05-20T11:27:17+0000",
            "type": "release"
        }"#;

        // Parse the JSON
        let result = ModernFabricManifest::from_json(sample_json);
        assert!(
            result.is_ok(),
            "Failed to parse Modern Fabric manifest: {:?}",
            result.err()
        );
        ...
        assert_eq!(manifest.inherits_from, "1.21.2");
        assert_eq!(
            manifest.main_class,
            "net.fabricmc.loader.impl.launch.knot.KnotClient"
        );
        assert_eq!(manifest.id, "fabric-loader-0.16.14-1.21.2");
        assert_eq!(manifest.release_type, "release");

        // Test libraries
        assert_eq!(manifest.libraries.len(), 3);
        ...
        // Test fabric loader library
        let loader_lib = &manifest.libraries[2];
        assert_eq!(loader_lib.name, "net.fabricmc:fabric-loader:0.16.14");

        // Test arguments
        assert_eq!(manifest.arguments.jvm.len(), 1);
        assert_eq!(
            manifest.arguments.jvm[0],
            "-DFabricMcEmu= net.minecraft.client.main.Main "
        );
        assert_eq!(manifest.arguments.game.len(), 0);

        // Test helper methods
        assert_eq!(manifest.get_minecraft_version(), "1.21.2");
        assert_eq!(manifest.get_loader_version(), Some("0.16.14"));

        // Test serialization roundtrip
        let serialized = manifest.to_json().expect("Failed to serialize manifest");
        let deserialized =
            ModernFabricManifest::from_json(&serialized).expect("Failed to deserialize manifest");
    ...
    fn test_empty_or_invalid_json() {
        ...
        let result = ModernFabricManifest::from_json("");
        assert!(result.is_err());

        // Test with malformed JSON
        let result = ModernFabricManifest::from_json("{\"inheritsFrom\": \"1.21.2\"");
        assert!(result.is_err());
